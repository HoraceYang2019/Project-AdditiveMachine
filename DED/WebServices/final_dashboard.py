...
try:
    from WebServices import decision_runtime
except ModuleNotFoundError:
    import decision_runtime

try:
    from DataPreprocess import alignment_runtime
except ModuleNotFoundError:
    import alignment_runtime

DATA_DIR = PROJECT_ROOT / "Data"
UI_DIR = PROJECT_ROOT / "UserInterfaceDesign"
PREPROCESS_DIR = PROJECT_ROOT / "DataPreprocess"
ONTOLOGY_DIR = PROJECT_ROOT / "LPBFOntology"

BASE_DIR = PROJECT_ROOT
OUTPUT_ROOT = DATA_DIR / "output"
DEFAULT_OUTPUT_NAME = "202603237504150010_modified"
TEMPLATES_DIR = UI_DIR / "dashboard_templates"
STATIC_DIR = UI_DIR / "dashboard_static"
UPLOAD_ROOT = DATA_DIR / "uploaded_inputs"
GENERATED_MPF_ROOT = DATA_DIR / "generated_mpf"
EXPORTED_MPF_ROOT = DATA_DIR / "exported_mpf"
THERMAL_EXAMPLE_PATH = PREPROCESS_DIR / "examples" / "thermal-imager.example.json"
RECORDER_JSON_DIR = DATA_DIR / "json"
THERMAL_REFERENCE_DIR = DATA_DIR / "csv"
...
def get_output_sensor_binding(output_dir: Path, sensor_name: str) -> dict[str, Any]:
    bundle = load_output_sensor_bundle(output_dir)
    file_name = str(bundle.get(f"{sensor_name}_file") or "").strip()
    candidate_path = output_dir / "uploaded_sources" / Path(file_name).name if file_name else None
    time_field = str(bundle.get(f"{sensor_name}_time_field") or "").strip() or None
    value_field = str(bundle.get(f"{sensor_name}_value_field") or "").strip() or None
    return {
        "sensor_name": sensor_name,
        "declared": bool(file_name),
        "file_name": Path(file_name).name if file_name else None,
        "path": candidate_path if candidate_path is not None and candidate_path.is_file() else None,
...
def annotate_sensor_source_issue(
    ...
    *,
    ...
    if source_file is not None:
        payload["source_file"] = Path(str(source_file)).name if str(source_file).strip() else "-"
...
def summarize_output_sensor_bundle(
    output_dir: Path,
    ...
    bundle = load_output_sensor_bundle(output_dir)
    thermal_binding = get_output_sensor_binding(output_dir, "thermal")
    edge_binding = get_output_sensor_binding(output_dir, "edge")
    mpf_file = str(bundle.get("mpf_file") or "").strip()
    declared_count = sum(
        1
        for item in (
            bool(mpf_file),
            thermal_binding["declared"],
            edge_binding["declared"],
        )
        if item
    )
    return {
        ...
        "complete": declared_count == 3,
        "mpf": {
            "declared": bool(mpf_file),
            "file_name": Path(mpf_file).name if mpf_file else None,
        },
        "thermal": {
            "declared": bool(thermal_binding["declared"]),
            "file_name": thermal_binding["file_name"],
            "exists": thermal_binding["path"] is not None,
            "time_field": thermal_binding["time_field"],
            "value_field": thermal_binding["value_field"],
            "active_source_file": str(thermal.get("source_file") or "") or None,
            "sample_count": int(thermal.get("sample_count") or 0),
            "source_error": str(thermal.get("source_error") or "") or None,
        },
        "edge": {
            "declared": bool(edge_binding["declared"]),
            "file_name": edge_binding["file_name"],
            "exists": edge_binding["path"] is not None,
            "time_field": edge_binding["time_field"],
            "value_field": edge_binding["value_field"],
            "active_source_file": str(edge.get("source_file") or "") or None,
            "sample_count": int(edge.get("sample_count") or 0),
            "has_machine_coordinates": bool(edge.get("has_machine_coordinates")),
            "source_error": str(edge.get("source_error") or "") or None,
        },
        "recorder_timing": {
            "available": bool(recorder_timing.get("available")),
            "source_file": str(recorder_timing.get("source_file") or "") or None,
        },
    }
...
def find_accurate_thermal_source(output_dir: Path) -> Path | None:
    thermal_binding = get_output_sensor_binding(output_dir, "thermal")
    if thermal_binding["declared"]:
        return thermal_binding["path"]
...
def load_saved_thermal_data(output_dir: Path, program_id: str) -> dict[str, Any]:
    thermal_binding = get_output_sensor_binding(output_dir, "thermal")
    if thermal_binding["declared"]:
        if thermal_binding["path"] is None:
            return annotate_sensor_source_issue(
                make_empty_thermal_data(program_id),
                source_file=thermal_binding["file_name"],
                source_kind="uploaded",
                source_error="The bundled thermal source file is missing.",
            )
        try:
            return build_thermal_dataset(
                thermal_binding["path"],
                program_id,
                time_field=thermal_binding["time_field"],
                value_field=thermal_binding["value_field"],
                source_kind="uploaded",
            ...
            return annotate_sensor_source_issue(
                ...
                source_file=thermal_binding["file_name"],
                source_kind="uploaded",
                source_error=f"Failed to parse the bundled thermal source: {exc}",
            )
...
def load_saved_edge_data(output_dir: Path, program_id: str) -> dict[str, Any]:
    edge_binding = get_output_sensor_binding(output_dir, "edge")
    if edge_binding["declared"]:
        if edge_binding["path"] is None:
            return annotate_sensor_source_issue(
                make_empty_edge_data(program_id),
                source_file=edge_binding["file_name"],
                source_kind="uploaded",
                source_error="The bundled edge source file is missing.",
            )
        try:
            return build_edge_dataset(
                edge_binding["path"],
                program_id,
                time_field=edge_binding["time_field"],
                value_field=edge_binding["value_field"],
            ...
            return annotate_sensor_source_issue(
                ...
                source_file=edge_binding["file_name"],
                source_kind="uploaded",
                source_error=f"Failed to parse the bundled edge source: {exc}",
            )
...
def find_recorder_timing_source(output_dir: Path) -> Path | None:
    edge_binding = get_output_sensor_binding(output_dir, "edge")
    if edge_binding["declared"]:
        bundled_path = edge_binding["path"]
...
def clone_sensor_payloads(source_output_dir: Path, target_output_dir: Path, program_id: str) -> None:
    ...
    if "thermal_time_field" in bundle:
        copied_fields["thermal_time_field"] = str(bundle.get("thermal_time_field") or "")
    if "thermal_value_field" in bundle:
        copied_fields["thermal_value_field"] = str(bundle.get("thermal_value_field") or "")
    ...
    if "edge_time_field" in bundle:
        copied_fields["edge_time_field"] = str(bundle.get("edge_time_field") or "")
    if "edge_value_field" in bundle:
        copied_fields["edge_value_field"] = str(bundle.get("edge_value_field") or "")
...
def process_mpf_text_to_new_output(
    ...
    from DataPreprocess.parse_mpf_to_json import process_file
...
def handle_upload_request(form: cgi.FieldStorage) -> dict[str, Any]:
    ...
    if thermal_item is not None:
        ...
        update_output_sensor_bundle(
            output_dir,
            thermal_file=thermal_source_path.name,
            thermal_time_field=thermal_time_field or "",
            thermal_value_field=thermal_value_field or "",
        )
    ...
    if edge_item is not None:
        ...
        update_output_sensor_bundle(
            output_dir,
            edge_file=edge_source_path.name,
            edge_time_field=edge_time_field or "",
            edge_value_field=edge_value_field or "",
        )
...
def build_inference_output_contract() -> dict[str, Any]:
    return {
        ...
        "description": "播放到任一時間點時，用同一份格式描述目前狀態、原因、信心與建議調整。",
        "required_fields": [
            "state",
            "cause",
            "confidence",
...
def build_default_inference_result() -> dict[str, Any]:
    return {
        ...
        "state": "not_evaluated",
        "state_label": "尚未推論",
        "cause": "等待 Heat Playback 選定時間點後，依固定格式輸出推論結果。",
        "confidence": 0.0,
        "recommended_action": "目前已建立 InferenceResult 格式，下一步才會把 rule table 轉成即時計算。",
        "recommended_parameter_change": [],
        "related_rule": None,
        "related_parameter": [],
        "evidence": {
...
def build_inference_rule_table(
    ...
    def has_parameter_value(field_name: str) -> bool:
    ...
    def has_toolpath_value(field_name: str) -> bool:
        for segment in toolpath_segments:
    ...
    available_fields = [
        ...
        },
        {
            ...
            "field": "laser_power_w",
            "label": "雷射功率",
            "available": has_parameter_value("laser_power_w"),
        },
        {
            "source": "mpf",
            "field": "spot_diameter_mm",
            "label": "光斑直徑",
            "available": has_parameter_value("spot_diameter_mm"),
        },
        {
            "source": "mpf",
            "field": "powder_supply_on",
            "label": "送粉狀態",
            "available": has_parameter_value("powder_supply_on"),
        },
        {
            "source": "mpf",
            "field": "dwell_s",
            "label": "停留時間",
            "available": has_parameter_value("dwell_s"),
        },
        {
            "source": "toolpath",
            "field": "feed_rate_mm_min",
            "label": "進給速度",
            "available": has_toolpath_value("feed_rate_mm_min"),
        },
    ]

    return {
        "module_id": "ontology-inference-rule-table",
        "title": "製程參數建議",
        "subtitle": "依目前 Heat Playback 位置，直接顯示狀態、原因與可調整的製程參數。",
        "ontology_sources": [
        ...
        "decision_basis": [
            {
                ...
                "label": "絕對溫度門檻",
                "description": "先用 G_High 的絕對值區分優良、警示與異常。",
            },
            {
        ...
        "identification_output_contract": inference_runtime.build_identification_output_contract(),
        "inference_output_contract": inference_runtime.build_inference_output_contract(),
        ...
        "rules": [
            {
                ...
                "cause_hint": "預期為正常沉積與正常冷卻。",
                "recommended_adjustments": [
                    "維持目前雷射功率與進給速度。",
                    "持續監看同 layer 後段是否出現持續升溫或延遲降溫。",
                ],
                "recommended_parameter_change": [
                    {
            ...
            },
            {
                ...
                "range_label": "1300 至 1500 °C",
                "diagnosis": "熱輸入偏高或偏不穩，應對照同參數與同路徑基準做微調。",
                "cause_hint": "可能是熱累積、進給偏慢、能量密度偏高，或送粉/噴嘴狀態開始漂移。",
                "recommended_adjustments": [
                    "若同參數基準也偏高，雷射功率可先下修 2% 至 5%。",
                    "若路徑位置固定偏熱，進給速度可上調 2% 至 5%。",
                    "若升溫延遲或波形異常，優先檢查送粉、噴嘴與光斑。",
                ],
                "recommended_parameter_change": [
                    {
                        "parameter": "laser_power_w",
            ...
            },
            {
                ...
                "range_label": ">= 1500 °C",
                "diagnosis": "熱輸入過高，應立即標記異常區間並啟動參數修正。",
                "cause_hint": "可能是雷射功率過高、進給過慢、熱累積嚴重，或送粉與焦點條件異常。",
                "recommended_adjustments": [
                    "雷射功率優先下修 5% 至 10%，並同步評估進給速度上調 5% 至 8%。",
                    "檢查送粉是否異常、噴嘴是否阻塞，以及焦點與光斑是否漂移。",
                    "保留該區段的時間戳、路徑位置與參數快照，作為後續 inference 與 adaptation 依據。",
                ],
                "recommended_parameter_change": [
                    {
                        "parameter": "laser_power_w",
...
def build_dashboard_payload(output_name: str | None = None) -> dict[str, Any]:
    ...
    if accurate_thermal_source is not None:
        try:
            ...
            accurate_thermal_reference = None
    edge = alignment_runtime.apply_time_alignment(
        edge=edge,
        recorder_timing=recorder_timing,
        accurate_thermal_reference=accurate_thermal_reference,
    )
    ...
    if accurate_thermal_reference is not None:
        thermal = normalize_loaded_thermal_payload(
            accurate_thermal_reference,
            program_id,
            "reference",
        )
    alignment, coordinate_alignment = alignment_runtime.build_alignment_views(
        thermal=thermal,
        edge=edge,
        toolpath_segments=toolpath_segments,
    )
    sensor_bundle = summarize_output_sensor_bundle(output_dir, thermal, edge, recorder_timing)
    decision_rule_table = decision_runtime.build_decision_rule_table(
        thermal,
        edge,
        parameter_events,
        toolpath_segments,
    ...
    header_cards.append({"label": "資料綁定", "value": f'{sensor_bundle["declared_count"]}/3'})
    if edge.get("sample_count", 0):
    ...
    return {
        ...
        "inference_rule_table": decision_rule_table,
        "decision_rule_table": decision_rule_table,
        "identification_output_contract": decision_runtime.build_identification_output_contract(),
        "identification_current_result": decision_runtime.build_default_identification_result(),
        "inference_output_contract": decision_runtime.build_inference_output_contract(),
        "inference_current_result": decision_runtime.build_default_inference_result(),
        "decision_output_contract": decision_runtime.build_decision_output_contract(),
        "decision_current_result": decision_runtime.build_default_decision_result(),
        "output_name": output_dir.name,
