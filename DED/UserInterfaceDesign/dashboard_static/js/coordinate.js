export function initCoordinateSection(ctx) {
    ...
    function shouldUseEdgeHeatTrace(edgeTrace) {
        ...
        const featureGap = Math.abs(Number(mapping.feature_gap_s));
        if (Number.isFinite(featureGap) && featureGap > 6) {
            return false;
        }
        return true;
    }

    function alignmentActiveWindow() {
        const activeWindow = ctx.state.alignment?.thermal_active_window;
        if (!activeWindow) {
            return null;
        }
        const manualOffsetMs = Number(ctx.alignmentManualOffsetMs || 0);
        const startTimestampMs = Number(activeWindow.aligned_start_timestamp_ms ?? activeWindow.start_timestamp_ms);
        const endTimestampMs = Number(activeWindow.aligned_end_timestamp_ms ?? activeWindow.end_timestamp_ms);
        if (!Number.isFinite(startTimestampMs) || !Number.isFinite(endTimestampMs) || endTimestampMs < startTimestampMs) {
            return null;
        }
        return {
            start_timestamp_ms: startTimestampMs + manualOffsetMs,
            end_timestamp_ms: endTimestampMs + manualOffsetMs,
        };
    }

    function clipTraceToAlignmentWindow(trace) {
        if (!Array.isArray(trace) || !trace.length) {
            return trace;
        }
        const activeWindow = alignmentActiveWindow();
        if (!activeWindow) {
            return trace;
        }
        const windowDuration = Math.max(
            Number(activeWindow.end_timestamp_ms) - Number(activeWindow.start_timestamp_ms),
            1,
        );
        const marginMs = ctx.clamp(windowDuration * 0.08, 500, 4000);
        const filtered = trace.filter((point) => (
            Number(point.timestamp_ms) >= Number(activeWindow.start_timestamp_ms) - marginMs
            && Number(point.timestamp_ms) <= Number(activeWindow.end_timestamp_ms) + marginMs
        ));
        return filtered.length >= Math.max(6, Math.floor(trace.length * 0.15)) ? filtered : trace;
    }
    ...
    function coordinateHeatTrace() {
        ...
        if (shouldUseEdgeHeatTrace(edgeSamples)) {
            return clipTraceToAlignmentWindow(edgeSamples
        ...
        return clipTraceToAlignmentWindow(mergedSamples);
    }
    ...
    function processStartTimestampMs(heatTrace) {
        ...
        const activeStartTimestamp = Number(activeWindow?.start_timestamp_ms);
        if (!Array.isArray(heatTrace) || !heatTrace.length) {
            return null;
        }
        ...
        const candidates = [];
        if (Number.isFinite(processTimestamp) && processTimestamp >= minTimestamp && processTimestamp <= maxTimestamp) {
            candidates.push(processTimestamp);
        }
        if (Number.isFinite(activeStartTimestamp) && activeStartTimestamp >= minTimestamp && activeStartTimestamp <= maxTimestamp) {
            candidates.push(activeStartTimestamp);
        }
    ...
    function renderCoordinateAlignment() {
        ...
        if (!geometry) {
            ...
            updateCoordinateControls(null, null);
            renderCoordinateAlertPanel(null, null);
            if (typeof ctx.updateInferencePlaybackResult === "function") {
                ctx.updateInferencePlaybackResult(null, null);
            }
            return;
        }
        ...
        if (typeof ctx.updateInferencePlaybackResult === "function") {
            ctx.updateInferencePlaybackResult(geometry, snapshot);
        }
    }
